...
if not API_TOKEN:
    raise RuntimeError(
        "BOT_TOKEN табылмады. Hosting Environment Variables бөліміне "
        "BOT_TOKEN қосыңыз."
    )
...
async def init_db():
    async with aiosqlite.connect(DB) as db:
        ...
        # Ескі DB үшін миграция
        try:
            await db.execute(
                "ALTER TABLE content ADD COLUMN file_unique_id TEXT"
            )
        except Exception:
            pass
        ...
        try:
            await db.execute(
                "ALTER TABLE submissions ADD COLUMN file_unique_id TEXT"
            )
        except Exception:
            pass

        # Duplicate video болмауы үшін
        await db.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_content_unique
            ON content(file_unique_id)
        """)

        await db.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_submissions_unique
            ON submissions(file_unique_id)
        """)
...
async def get_user(uid: int):
    async with aiosqlite.connect(DB) as db:
        return await (
            await db.execute(
                """
                SELECT balance, last_bonus, last_active, vip_until
                FROM users
                WHERE id=?
                """,
                (uid,)
    ...
    async def on_process_message(
        ...
        if not await check_sub(message.from_user.id):
            ...
            await message.answer(
                ...
                "Ботты қолдану үшін алдымен біздің "
                "каналға тіркеліңіз.",
        ...
        if not await check_sub(call.from_user.id):
            ...
            try:
                await bot.send_message(
                    call.from_user.id,
                    "⚠️ <b>Ботты қолдану үшін каналға "
                    "тіркеліңіз:</b>",
                    reply_markup=subscription_kb()
                )
            except Exception:
...
@dp.message_handler(
    lambda m: m.text == "✅ Аяқтау",
    state=[
        AdminStates.add_v_file,
        UserStates.upload_video
    ]
)
async def finish_upload(
    ...
    text = (
        "📊 <b>Жүктеу нәтижесі:</b>\n\n"
        f"✅ Қабылданды: <b>{added}</b>\n"
        f"♻️ Қайталанған: <b>{dupes}</b>"
    )
...
async def start(
    ...
    if not await check_sub(uid):
        ...
        await message.answer(
            "👋 <b>Сәлем!</b>\n\n"
            "Ботты қолдану үшін каналға "
            "міндетті түрде тіркеліңіз.",
            reply_markup=subscription_kb()
        )
...
async def add_video_start(
    ...
    await message.answer(
        "📂 <b>Видео жанрын таңдаңыз:</b>",
        reply_markup=genre_kb(include_vip=True)
    )


@dp.message_handler(
    state=AdminStates.add_v_genre,
    user_id=ADMIN_ID
)
async def add_video_genre(
    message: types.Message,
    state: FSMContext
):
...
@dp.message_handler(
    lambda m: m.text == "➕ Видео жіберу"
)
async def user_upload_start(
    message: types.Message
):
    ...
    await message.answer(
        "📂 <b>Қай жанрға видео жібересіз?</b>",
...
@dp.message_handler(
    state=UserStates.upload_genre
)
async def user_upload_genre(
    message: types.Message,
    state: FSMContext
):
...
@dp.message_handler(
    state=UserStates.upload_video,
    content_types=["video"]
)
async def user_upload_video(
    message: types.Message,
    state: FSMContext
):
